...
| # | 시나리오 | 단계 | 기대 결과 |
|---|---|---|---|
| **C1** | 도구 목록 | Inspector에서 **Tools** 탭으로 전환 | `chat-completions` 1개가 입력 스키마(model, messages, temperature, max_tokens, top_p, stop)와 함께 표시 |
| **C2** | Happy path | `chat-completions`에서 **Run Tool** 클릭. 입력: `model: gpt-4o-mini`, `messages: [{role: "user", content: "ping"}]` | 응답이 `result.content[0].text`에 누적 텍스트 포함. `result.structuredContent.usage.total_tokens > 0`. `result.isError`는 `false`. |
...
| **C7** | 다중 등록 *(SDK 컨슈머 전용)* | 서버가 `registerOpenAIChat`을 두 이름(예: `chat-completions` + `azure_chat`, 서로 다른 `apiKey` + `baseURL`)으로 등록한 경우. **Tools** 탭에서 각각 실행. | `tools/list`에 두 entry 모두 표시. 각 `tools/call`이 자기 업스트림에서 응답 (호출 사이에 업스트림을 바꿔보며 응답이 섞이지 않는지 확인). |
