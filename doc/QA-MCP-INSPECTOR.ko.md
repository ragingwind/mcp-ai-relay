...
```bash
pnpm inspect                                  # tools/call → openai_chat ("ping")
...
등록한 경우에만 해당합니다 (v1 기본 릴레이는 단일 `openai_chat`만
등록하므로, C7은 SDK의 `multi-registration` 예제 또는 다중 업스트림을
한 서버에 등록한 컨슈머에서 검증).

| # | 시나리오 | 단계 | 기대 결과 |
|---|---|---|---|
| **C1** | 도구 목록 | Inspector에서 **Tools** 탭으로 전환 | `openai_chat` 1개가 입력 스키마(model, messages, temperature, max_tokens, top_p, stop)와 함께 표시 |
| **C2** | Happy path | `openai_chat`에서 **Run Tool** 클릭. 입력: `model: gpt-4o-mini`, `messages: [{role: "user", content: "ping"}]` | 응답이 `result.content[0].text`에 누적 텍스트 포함. `result.structuredContent.usage.total_tokens > 0`. `result.isError`는 `false`. |
